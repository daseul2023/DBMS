...
-- 내가 푼 문제
select dno 부서번호, sum(salary) 합계, round(avg(salary),2) 평균, max(salary) 최대값, min(salary) 최소값
from employee
...
-- 선생님이 푼 문제
select dno 부서번호, sum(salary) 합계, round(avg(salary),2) 평균, max(salary) 최대값, min(salary) 최소값, count(*)
from employee
where dno <> 10         -- !=, ^=
...
-- 2.  직책의 SALSMAN, PRESIDENT, CLERK 을 제외한 각 부서별 월급의 합계와 평균과 최대값, 최소값을 구하시오.  
select job 직책, sum(salary) 합계, round(avg(salary),2) 평균, max(salary) 최대값, min(salary) 최소값
from employee
where job not in ('SALESMAN', 'PRESIDENT', 'CLERK')
group by job;            -- 같은 부서를 그룹핑

-- 3. SMITH 과 동일한 부서에 근무하는 사원들 의 월급의 합계와 평균과 최대값, 최소값을 구하시오. 
select dno 부서, sum(salary) 합계, round(avg(salary),2) 평균, max(salary) 최대값, min(salary)최소값
from employee
...
-- 8. sub query - 부서별로 최소 급여를 받는 사용자의 이름과 급여와 직책과 부서번호를 출력하세요. 
select ename 사용자이름, salary 급여, job 직책, dno 부서번호
from employee
...
select ename 사용자이름, salary 급여, job 직책, dno 부서번호
from employee 
where salary > (select round(avg(salary),2)from employee);            -- 전체 평균을 select하는 subquery 작성

-- 10. sub query - 급여가 평균 급여보다 많은 사원들의 사원번호와 이름을 표시하되 결과를 급여에 대해 오름차순 정렬하시오.
select ename 이름, dno 사원번호
from employee
