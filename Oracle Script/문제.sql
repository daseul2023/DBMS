...
-- <문제2> dno(부서번호) 가 20이고 입사날짜가 81년 4월 이후 사원의 이름과 직책과 입사날짜를 출력
select ename 사원이름, job 직책, hiredate 입사날짜, dno 부서번호, hiredate 입사날짜
from employee
...
select eno 사원번호, ename as 사원이름, salary 월급, commission 보너스, salary * 12 +nvl (commission, 0) 전체연봉
from employee
where salary * 12 +nvl (commission, 0) > 30000;

-- <문제4> commission이 null 이 아닌 사용자의 이름만 출력
select ename 사용자이름, commission 보너스
from employee
...
select ename 사원이름, hiredate 입사날짜
from employee
where hiredate like '81%';        -- 81년도에 있는 모든 입사 날짜가 찍힘.
...
-- <문제9> 월급이 1500 이하이면서 부서번호가 20번 이 아닌 사원이름과, 월급과, 부서번호를 출력
select ename 사원이름, salary as 월급, dno 부서번호
from employee
where salary <=1500 and dno != 20;

-- <문제10> 사원번호(eno) 가 7788, 7782 인 부서번호와 이름과 직책을 출력 
select dno 부서번호, ename 이름, job 직책, eno 사원번호
from employee
where eno like 7788 or eno like 7782;
